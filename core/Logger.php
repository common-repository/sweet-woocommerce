...
class Logger {
	...
	function __construct() {
		...
		add_filter( 'cron_schedules', function ( $schedules ) {
			$schedules['every-2-days'] = array(
					'interval' => 1440 * 2 * MINUTE_IN_SECONDS,
					'display'  => __( 'Every 2 days' )
			);
			return $schedules;
		});

		// Schedule cron to prune
		$scheduled = wp_next_scheduled( 'wp_logging_prune_routine' );
		if ( $scheduled == false ){
			wp_schedule_event( time(), 'every-2-days', 'wp_logging_prune_routine' );
		}
	}
	...
	private function get_logs_to_prune(){
		...
		$how_old = apply_filters( 'wp_logging_prune_when', '2 days ago' );
